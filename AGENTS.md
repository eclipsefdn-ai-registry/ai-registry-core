...
- **Marketplace sources** — a `marketplaces/*.json` approval trusts a vendor-published marketplace file (e.g. OpenAI Codex/ChatGPT's `.agents/plugins/marketplace.json`) instead of a single plugin. Every entry it lists is resolved and fanned out into its own plugin entry at consolidation time, tagged with a `sourcedFrom` provenance field — re-resolved on every run, so new entries the vendor adds to their marketplace file are picked up automatically. See `docs/superpowers/specs/2026-09-04-agent-plugin-marketplaces-design.md` for the full design.
