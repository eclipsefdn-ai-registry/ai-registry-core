...
- **source** (required): Object with `url` (git repo URL) and optionally `path`.
  ...
  - **ref** (optional): Git ref (tag, branch name, or full commit SHA) to check out instead of the repository's default branch. Omit to always track the default branch. Applies to every path a glob or array expands into.
