...
- Guidance for tools consuming the registry — as a [website page](https://ai.open-vsx.org/docs/clients) and as a [Claude Code skill](skills/implement-registry-client/SKILL.md)
...
A tool integration typically fetches `organizations.json` + its own `tools/<tool-id>.json`. See the [client guidance](skills/implement-registry-client/SKILL.md) for what to do with them: resolving approvals, showing who approved an artifact, verifying content, installing, and keeping it current.
...
- [Client guidance skill](skills/implement-registry-client/SKILL.md) — AI agent skill for implementing a registry client in a tool
